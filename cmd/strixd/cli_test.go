...
// withRules writes a one-rule dir and points MAILSTRIX_RULES_DIR at it for the test,
...
func withRules(t *testing.T, content string) {
	...
	t.Setenv("MAILSTRIX_RULES_DIR", dir)
...
func TestCheckRulesFail(t *testing.T) {
	...
	t.Setenv("MAILSTRIX_RULES_DIR", t.TempDir())
	t.Setenv("MAILSTRIX_RULES", "")
...
func TestScanStdin(t *testing.T) {
	withRules(t, eicarRule)
	// Feed the EICAR payload on stdin (the `strixd scan - < maildirfile` case).
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatal(err)
	}
...
// a temp cache dir, and NO MAILSTRIX_RULES/MAILSTRIX_RULES_DIR, `strixd scan` must seed
// the cache from the seed (exactly as `serve` does) so the daemon's Docker-image
// env — MAILSTRIX_SEED_RULES + MAILSTRIX_CACHE_DIR set, MAILSTRIX_RULES unset — works without
...
func TestScanSeedRulesSeeding(t *testing.T) {
	...
	t.Setenv("MAILSTRIX_SEED_RULES", seedPath)
	t.Setenv("MAILSTRIX_CACHE_DIR", cacheDir)
...
// .yac seed, a temp cache dir, and NO MAILSTRIX_RULES/MAILSTRIX_RULES_DIR, `check-rules`
