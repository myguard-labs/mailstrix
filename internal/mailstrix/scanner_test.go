...
func TestScannerRuleAllowlist(t *testing.T) {
	...
	// meta.mailstrix_allow="1" so the plugin can score it log-only — visibility
	...
	if m[0].Meta["mailstrix_allow"] != "1" {
		t.Errorf("allowlisted match not tagged mailstrix_allow=1: %+v", m[0])
	}
}
