...
// string. It shares the dispatch loop with parseBIFF8Formula (same operand
// stack, same ptgConcat/ptgFunc/ptgFuncVar handling, same caps and fail-open
// contract) via biff12Dialect; the divergences are ptgStr (BIFF12 uses a uint16
// charcount + UTF-16LE chars, no fHighByte flag) and the wider, bounds-checked
// reference-token payloads. Never panics.
func parseBIFF12Formula(data []byte) string {
	return foldBIFFPtgStream(data, biff12Dialect)
}
